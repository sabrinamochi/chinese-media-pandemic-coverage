...
var smallScreen, width, margin, height, side;
...
function drawChart() {
  ...
  var linearGradient1 = g.append("defs")
  ...
  var linearGradient1Back = g.append("defs")
    .append("linearGradient")
  ...
  var linearGradient2 = g.append("defs")
    .append("linearGradient")
  ...
  var linearGradient2Back = g.append("defs")
    .append("linearGradient")
  ...
  d3.queue()
    ...
    .await(function (error, data, data2) {
      ...
      var jan = data.filter(function (d) {
        return d.dateall == "2020-01"
      })
      var feb = data.filter(function (d) {
        return d.dateall == "2020-02"
      })
      var mar = data.filter(function (d) {
        return d.dateall == "2020-03"
      })
      ...
      r.domain([d3.min(jan, function (d) {
        return +d.normalizedTotal
      }), d3.max(mar, function (d) {
        return +d.normalizedTotal
      })])
      ...
      g.selectAll(".first-part-line")
        .data(groupWord, function (d) {
          return d.word;
        })
        .enter()
        .append("line")
      ...
      var janPoint = g.selectAll(".jan-point")
        .data(jan)
        .enter().append("circle")
        .attr("class", "corpus-node jan-point")

      var febPoint = g.selectAll(".feb-point")
        .data(feb)
        .enter().append("circle")
        .attr("class", "corpus-node feb-point");


      var marPoint = g.selectAll(".mar-point")
        .data(mar)
        .enter().append("circle")
...
function updateDimensions() {
  ...
  width = document.querySelector("#triangle-graph").clientWidth * 0.95;
  margin = {
  ...
  height = document.querySelector("#triangle-graph").clientHeight - margin.top - margin.bottom
  side = height * 2 / Math.sqrt(3)
  svg
    .attr("width", width)
    .attr("height", height + margin.top + margin.bottom)

  g.attr("transform", `translate(${(width - side) / 3}, ${(margin.top + 0.5)})`)
  ...
  r.range([smallScreen ? 1 : 2, smallScreen ? 11 : 18]);
...
$corpusWord5.waypoint(function (direction) {
  ...
  if (direction == "down") {
    ...
    var val = groupWord.find(function (item) {
      return item.word == selWord
    });
